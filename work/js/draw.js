...
class DrawManager {
    ...
    // 绘制主要地图内容
    drawMap(type, background, collidable, tram, app, events) {
        ...
        // 绘制相反属性的虚化砖块
        this.drawPhantomBlocks(collidable[detype]);

        // 绘制当前属性的碰撞元素
        this.drawCollidableElements(collidable[type]);
    ...
    // 绘制虚化砖块（相反属性的碰撞箱）
    drawPhantomBlocks(phantomCollidable) {
        for (let i of phantomCollidable) {
    ...
    // 绘制碰撞元素
    drawCollidableElements(collidable) {
        for (let i of collidable) {
            const ctx = this.game.ctx;
            const { x, y, w, h } = i;
            ...
            if (i instanceof Tile) {
                if (i.img.length == 0) {
                    // 石板砖块效果
                    this.drawStoneTile(ctx, x, y, w, h);
                } else {
                    // 绘制贴纸/装饰图层
                    this.drawImageTile(ctx, i, x, y, w, h);
                }
    ...
    // 绘制图片砖块
    drawImageTile(ctx, tile, x, y, w, h) {
        if (tile.hp) {
            if (this.game.changetimes == 0 || !this.game.mapmanager.hurt()) {
                ...
                console.warn(tile.hp, tile.hp - Math.floor(this.game.changetimes / 2));
                ctx.drawImage(
                    tile.img[
                        Math.max(
                            0,
    ...
    // 绘制血条
    async drawHP(collidable, HP, env) {
